...
- 캐릭터 조작 (드래그로 이동, 슬라이더로 크기 조절, 회전, 좌우반전)
- 사진 촬영
- 캐릭터별 촬영 완료 안내문구 표시
- 사진 저장 및 공유
...
### 캐릭터 텍스트 및 설정
- 각 페이지(`characterN.html`) 하단의 `window.characterData` 값을 수정해 캐릭터 이름, 인사말, 안내 문구, 장소 정보를 변경할 수 있습니다.
- `#character` 요소의 `data-photo-guidance` 속성과 `<img>` 경로 또한 페이지별로 조정 가능합니다.
